...
import { uploadModelFile, uploadThumbnail } from "@/lib/storage";
import { createModel } from "@/lib/models";
import { useDropzone } from "react-dropzone";
import {
  Upload, CheckCircle, AlertCircle, FileBox,
  Tag, DollarSign, ChevronRight, X, Info,
  Layers, Globe, Lock
} from "lucide-react";
...
interface ModelMeta {
  ...
  basePrice: string;
  isFree: boolean;
  weightGrams: string;      // zorunlu - gram
  dimensionX: string;       // opsiyonel - mm
  dimensionY: string;       // opsiyonel - mm
  dimensionZ: string;       // opsiyonel - mm
}
...
const LICENSES = [
  {
    value: "standard",
    label: "Standart",
    desc: "Alıcı yalnızca 1 baskı alabilir.",
    icon: Lock,
  },
  {
    value: "multi_print",
    label: "Çoklu Baskı",
    desc: "Alıcı birden fazla baskı alabilir.",
    icon: Layers,
  },
  {
    value: "open",
    label: "Açık",
    desc: "Herkes ücretsiz olarak indirebilir.",
    icon: Globe,
  },
] as const;
...
export function UploadPageClient() {
  ...
  /* ── Dropzone ─────────────────────────────────────── */
  const onDrop = useCallback((accepted: File[], rejected: { file: File; errors: readonly { code: string; message: string }[] }[]) => {
    setFileError("");
    if (rejected.length > 0) {
      const err = rejected[0].errors[0];
      if (err.code === "file-too-large") {
        setFileError(`Dosya çok büyük. Maksimum ${MAX_SIZE_MB} MB.`);
      } else {
        setFileError("Geçersiz format. STL, OBJ veya 3MF yükleyin.");
      }
      return;
    }
    if (accepted[0]) {
  ...
  async function handlePublish() {
    ...
    try {
      ...
      if (!user) {
        window.location.href = "/tr/auth/login?redirect=/tr/upload";
        return;
      }

      // Generate a temp model ID for storage path
      ...
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="text-center max-w-sm">
          ...
          <p className="text-sm text-[var(--text-secondary)] mb-6">
            ...
            Onaylandıktan sonra platformda yayınlanacak.
          </p>
          <div className="flex gap-3 justify-center">
            ...
            <button
              onClick={() => { setUploadDone(false); setFile(null); setStep("file"); setMeta({ title:"",description:"",category:"",tags:"",license:"standard",basePrice:"",isFree:false,weightGrams:"",dimensionX:"",dimensionY:"",dimensionZ:"" }); }}
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--text-tertiary)] mt-1">
          STL, OBJ veya 3MF dosyanızı yükleyin, yapılandırın ve yayınlayın.
        </p>
      </div>
            ...
            <button
              ...
              className={cn(
                ...
                s.id === step
                  ...
                  : i < stepIdx
                  ? "text-[#FF6B35] cursor-pointer hover:underline"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-4">
          <div className="h-80 lg:h-[420px] bg-[var(--bg-secondary)] border border-[var(--border)] rounded-2xl overflow-hidden">
            <ModelViewer file={file ?? undefined} toolbar />
          </div>
            ...
            <div className="flex items-center gap-3 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-[var(--text-tertiary)]">
                  {fileSizeMB} MB · {file.name.split(".").pop()?.toUpperCase()}
                </div>
              </div>
              <button
          ...
          {step === "file" && (
            <div className="flex flex-col gap-4">
              ...
              <div
                {...getRootProps()}
                className={cn(
                  "border-2 border-dashed rounded-2xl p-10 flex flex-col items-center text-center cursor-pointer transition-all",
                  isDragActive
              ...
              <div className="bg-[rgba(255,107,53,0.04)] border border-[rgba(255,107,53,0.15)] rounded-xl px-4 py-3 flex gap-2 text-xs text-[var(--text-secondary)]">
                ...
                <span>Model yüklendikten sonra 3D önizleme solda görünecektir. Yüklemeden önce modelin basılabilir olduğundan emin olun.</span>
              </div>
            </div>
          )}
          ...
          {step === "details" && (
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="örn: Araç Organizeri Pro"
                  value={meta.title}
              ...
              <div>
                ...
                <textarea
                  placeholder="Modelinizi tanımlayın — kullanım amacı, boyutlar, özellikler..."
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="raf, organizasyon, mutfak (virgülle ayır)"
                  value={meta.tags}
              ...
              <div>
                <label className="text-xs text-[var(--text-tertiary)] block mb-1.5">
                  Boyutlar (mm) — <span className="text-[var(--text-tertiary)] font-normal">opsiyonel</span>
                </label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <input
                      ...
                      min="0"
                      value={(meta as any)[d.key]}
                      onChange={(e) => updateMeta(d.key as any, e.target.value)}
                      className="w-full h-10 px-3 text-sm rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)] text-[var(--text-primary)] outline-none focus:border-[#FF6B35] transition-colors placeholder:text-[var(--text-tertiary)]"
                    />
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-2">
                  {LICENSES.map((lic) => {
                    ...
                    return (
                      <button
                        key={lic.value}
                        onClick={() => updateMeta("license", lic.value)}
                        className={cn(
                          "flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all",
                          meta.license === lic.value
              ...
              <div>
                ...
                <label className="flex items-center gap-2.5 mb-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={meta.isFree}
                    onChange={(e) => updateMeta("isFree", e.target.checked)}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-2xl overflow-hidden">
                {[
                  ...
                  { label: "Boyut",       value: fileSizeMB ? `${fileSizeMB} MB` : "—" },
                  ...
                  { label: "Etiketler",   value: meta.tags || "—" },
                  { label: "Ağırlık",     value: meta.weightGrams ? `${meta.weightGrams} gram` : "—" },
                  { label: "Boyutlar",    value: [meta.dimensionX, meta.dimensionY, meta.dimensionZ].filter(Boolean).length > 0 ? `${meta.dimensionX||"?"}×${meta.dimensionY||"?"}×${meta.dimensionZ||"?"} mm` : "—" },
                ].map((row, i) => (
                  <div key={row.label} className={cn("flex items-center gap-3 px-4 py-3", i !== 0 && "border-t border-[var(--border)]")}>
                    ...
                    <span className="text-sm text-[var(--text-primary)] truncate">{row.value}</span>
                  </div>
                ))}
              </div>
