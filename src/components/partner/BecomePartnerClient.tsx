...
export function BecomePartnerClient() {
  ...
  async function handleSubmit() {
    ...
    if (!user) {
      router.push(`/${locale}/auth/login?redirect=/become-partner`);
      return;
    }
    ...
    await (supabase as any).from("profiles").update({
      ...
      city: form.city,
      bio: `Yazıcılar: ${form.printerTypes.join(",")} | Malzemeler: ${form.materials.join(",")} | Hacim: ${form.buildVolume} | Kapasite: ${form.dailyCapacity}/gün`,
    }).eq("id", user.id);
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="text-sm text-[var(--text-secondary)] mb-8">
          ...
          Onaylandıktan sonra sipariş havuzuna erişim sağlayacaksınız.
        </p>
        <button
  ...
  if (step === "form") {
    return (
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-10">
        <button onClick={() => setStep("info")} className="text-sm text-[var(--text-tertiary)] hover:text-[var(--text-primary)] mb-6 flex items-center gap-1">
          ← Geri
        </button>
        ...
        <div className="flex flex-col gap-5">
          {/* Printer type */}
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider text-[var(--text-tertiary)] block mb-2">Yazıcı Tipi * (çoklu seçilebilir)</label>
            <div className="flex flex-wrap gap-2">
              {PRINTER_TYPES.map((pt) => (
                <button key={pt} onClick={() => togglePrinterType(pt)}
                  className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${form.printerTypes.includes(pt) ? "border-[#FF6B35] bg-[rgba(255,107,53,0.08)] text-[#FF6B35]" : "border-[var(--border)] text-[var(--text-secondary)]"}`}>
                  {pt}
                </button>
              ))}
            </div>
          ...
          {/* Materials */}
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider text-[var(--text-tertiary)] block mb-2">Desteklediğiniz Malzemeler *</label>
            <div className="flex flex-wrap gap-2">
              {MATERIALS_LIST.map((m) => (
                <button key={m} onClick={() => toggleMaterial(m)}
                  className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${form.materials.includes(m) ? "border-[#FF6B35] bg-[rgba(255,107,53,0.08)] text-[#FF6B35]" : "border-[var(--border)] text-[var(--text-secondary)]"}`}>
                  {m}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <textarea
              rows={3}
              placeholder="3D baskı deneyiminiz, özel yetenekleriniz..."
              value={form.notes}
          ...
          <button
            ...
            className="w-full h-11 bg-[#FF6B35] text-white rounded-xl text-sm font-medium hover:bg-[#e85e2a] disabled:opacity-40 transition-colors"
          >
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-2xl p-8 text-center">
        <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-2">Nasıl çalışır?</h2>
        <div className="flex flex-col sm:flex-row justify-center gap-0 mt-6 mb-8">
